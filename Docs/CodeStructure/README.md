...
Designed from the current C4 model, [Database Design](../Database/README.md), [API Documentation](../API/README.md), and UI/UX design ([Information Architecture](../UI-UX/InformationArchitecture_HRM.md) → Screens Hierarchy → UI/UX) and Use Cases/User Stories for all five modules. The backend layout is **implemented** in `backend/` for the first four and matches [`BackendStructure.md`](BackendStructure.md); Contract Management's backend layout is designed only (see that document's dedicated section) — no code exists for it. The frontend layout is **design-only** for all five modules — no `frontend/` folder exists yet.

- [`FrontendStructure.md`](FrontendStructure.md) — React (`frontend/`), feature-based folders, covering all five modules. *Design only.*
- [`BackendStructure.md`](BackendStructure.md) — ASP.NET Core (`backend/`), covering all four implemented modules plus a target layout for Contract Management. *Implemented (except Contract Management).*
